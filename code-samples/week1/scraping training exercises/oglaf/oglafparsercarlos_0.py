...
def extract_archive(link):
	'''
	link should be the archive page of oglaf.
	1st. It gets all the clickable links in the archive page.
	2nd. The function goes inside the link then saves the comic there to local,
	3rd. The function then checks if the comic has multiple parts that can only be downloaded on the pages after the current.

		If there are, it goes through all those pages, saves them to local, until it returns False.
		If there isnt any other pages, the function goes back to "for i in comic:" to try to download the next comic.


	'''
	#tries to open the page with a 5 attempt limit. 
	html = get_page(link, 5)
 
	# if html is not false. This will only become false if get_page() from above did not return anything.
	if html:
		soup = BeautifulSoup(html)

		#find all tags <a href= ..
		comic = soup.findAll('a')
		for i in comic:
			# method 'get()'' can obtain data with a var=data format			
			# code below gets value of href in i.
			address = i.get('href')
